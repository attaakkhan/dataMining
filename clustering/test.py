...
def get_row(r,i):
   ...
   # print (i)
    ...
    if (i == 7):
        rr= r[0:l:5]+ r[0:l:4]
    if (i == 8):
        rr= r[0:l:4]+r[0:l:7]+r[0:l:13]
    if (i == 9):

        rr= r[0:l:4]+r[0:l:7]+r[0:l:13]
    if (i == 11):
        rr= list(r1 - set((r[0:l:5]+ r[0:l:4])))
    if (i == 12):
        rr= list(r1 - set((  r[0:l:3])))
    if (i == 13):
        rr= list(r1 - set((   r[0:l:2])))
    if (i == 14):
        rr=  list(r1 - set((r[0:2 * int(l / 3)])))
    if (i == 15):
        rr= list(r1 - set(( r[0:int(l/3)])))
    if (i == 16):
        rr= list(r1 - (set(r[0:l:4] + r[0:l:7] + r[0:l:13])))
    if (i == 17):
        rr= list(r1 - set(( r[int(l/3):l])))
    if (i == 18):
        rr= list(r1 - set(( r[int(l / 2):l])))
    if (i == 10):
    ...
    if (i == 24):
        rr= r[25:30]
    if (i == 25):
        rr= r[4:6]+r[7:8]+r[13:19]+r[23:27]
    if (i == 26):
        rr= r[1:4]+r[9:12]+r[18:23]+r[28:30]
    if (i == 27):
        rr= r[4:6]+r[13:21]+r[26:27]
    if (i == 28):
        rr= r[1:2]+r[5:7]+r[11:15]+r[21:27]
    if(len(rr)==0):
        return [0,0,0,0,0]
    return rr


def add_feature(df, feature_name, feature_function, con_list,test=False):
    df_rmax = df.copy()
    if(test==False):
...
def get_features1(df,test=False):


        con_list = []
        if(test==False):
            target = df['target']
        add_feature(df, 'rdis2', distance2, con_list,test)
        add_feature(df, 'rdis', distance, con_list,test)
        add_feature(df, 'rdis3', distance3, con_list,test)
        add_feature(df, 'rS10', slop10, con_list,test)
...
def bisectdb(df,test=False,testData=None):
    if(test==True):
        testData.insert(0, "pos", [i for i in range(0, len(testData))], True)
        poses = []
        reses = []
        dflist = []
        for i in range(0, 3):
            df1 = testData[testData['y_kmeans'] == i - 1].copy()
            pos = list(df1['pos'])
            poses.append(pos)
            ...
            df2 = df[df['y_kmeans'] == i - 1].copy()
            df2.drop(['y_kmeans'], inplace=True, axis=1)
            ...
            res = dbbtest(df2,df1, np.asarray(cccccc[i]))
...
df1.insert(0, "y_kmeans", labels, True)
#print(len(targets))
...
cols_col = ["series_{}".format(i) for i in range(1, 30)]
test_df = pd.read_csv('proj3_test.csv', names=cols_col, header=None)
test_df_1=test_df.copy()
NORMALIZER_1 = 'bfill'
NORMALIZER_2 = 'pad'

test_df_1.fillna(method=NORMALIZER_1, inplace=True)
test_df_1.fillna(method=NORMALIZER_2, inplace=True)
#print(df)
test_df_1 = get_features1(test_df_1,True)
import pickle
kmeans=None
# with open('trainkmean.pickle', 'rb') as f:
#     kmeans=pickle.load(f)
list2=['rdis_17',   'rS10_18',   'rdis3_17',  'rdis_18',   'rdis_17']

list3=['rdis_17',   'rS10_18',   'rdis3_17',  'rdis_18',   'rdis_17']
test_df_k=test_df_1[list3].copy()


test_df_1=test_df_1[list2]
#print(test_df_1)
...
#print(test_df_1)
scaler = StandardScaler()
x_scaled = scaler.fit_transform(test_df_1)
test_df_1 = pd.DataFrame(x_scaled, columns=list2)

test_df_1= normalize(test_df_1)
test_df_1 = pd.DataFrame(test_df_1, columns=list2)
...
test_df_1.insert(0, "y_kmeans", labels, True)




dbPredict=bisectdb(df1,True,test_df_1)
#print(dbPredict)
...
cols = df.columns
scaler = StandardScaler()
x_scaled = scaler.fit_transform(test_df_k)
test_df_k = pd.DataFrame(x_scaled, columns=list3)

kpedict = kmeans.predict(test_df_k)

kpedict=list(kpedict)
dbPredict=list(dbPredict)

for ii in range(0,len(dbPredict)):
    dbPredict[ii]=dbPredict[ii]+1
    kpedict[ii]=kpedict[ii]+1
#
# print(dbPredict)
# print(kpedict)
...
resDf.insert(0, "k", kpedict, True)
resDf.insert(0, "db", dbPredict, True)
#print (resDf)
resDf.to_csv('resultsP3.csv', header=False,index=False)
